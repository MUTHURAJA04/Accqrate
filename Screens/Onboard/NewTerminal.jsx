...
const NewTerminal = () => {
  ...
  const handleSave = () => {
    ...
    // Navigate back to BoardingTerminal with the device data
    navigation.navigate('boardingTerminal', {
      newDevice: {
        ...formData,
        id: isEdit && editDevice ? editDevice.id : Date.now().toString(), // Generate ID for new devices, preserve for edits
        isEdit: isEdit, // Flag to indicate edit mode
        // You can set CSID and EndPoint based on form data or preserve existing values
        hasCSID: isEdit && editDevice ? editDevice.hasCSID : (formData.hasCSID || false),
        hasEndPoint: isEdit && editDevice ? editDevice.hasEndPoint : (formData.hasEndPoint || false),
        // Set default invoice data if not provided
        lastInvoiceNo: isEdit && editDevice ? editDevice.lastInvoiceNo : (formData.lastInvoiceNo || 'N/A'),
        lastInvoiceDate: isEdit && editDevice ? editDevice.lastInvoiceDate : (formData.lastInvoiceDate || new Date().toISOString().split('T')[0]),
      }
    });
    ...
    Alert.alert(
      'Success',
      isEdit ? 'Device data has been updated successfully' : 'Device data has been saved successfully',
      [{ text: 'OK' }]
    );
  };
